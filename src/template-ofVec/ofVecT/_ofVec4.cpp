...
_ofVec4<T> _ofVec4<T>::getScaled( const T length ) const {
	...
	if( l > 0 )
		return _ofVec4<T>( (x/l)*length, (y/l)*length,
...
_ofVec4<T> _ofVec4<T>::getInterpolated( const _ofVec4<T>& pnt, T p ) const {
	return _ofVec4<T>( x*(1-p) + pnt.x*p,
					 y*(1-p) + pnt.y*p,
					 z*(1-p) + pnt.z*p,
					 w*(1-p) + pnt.w*p );
}
...
inline _ofVec4<T> _ofVec4<T>::getMiddle( const _ofVec4<T>& pnt ) const {
	return _ofVec4<T>( (x+pnt.x)/2.0f, (y+pnt.y)/2.0f,
...
_ofVec4<T> _ofVec4<T>::getNormalized() const {
	...
	if( length > 0 ) {
		return _ofVec4<T>( x/length, y/length, z/length, w/length );
	} else {
		return _ofVec4<T>();
	}
}
